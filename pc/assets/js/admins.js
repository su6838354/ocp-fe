...
function operateEvent(){
    ...
    $('#datatable .j_view').off().on('click', function (e) {
        e.preventDefault();
        window.open('admin.html?id='+$(this).parent().attr('data-pid')+'#user');
    });
}
...
function createLateEvent(){
    ...
    $('.type').change(function(e) {
        e.preventDefault();
        if($('.type').val()=="group"){
            $('.group_type').html('<option value="0">其他</option><option value="1">乡镇</option>')
        }else{
            $('.group_type').html('<option value="0">其他</option')
        }
    });
}
...
function createRow() {
    ...
    tempTRTDs.eq(8).html('<input class="tel" type="text" style="width: 70px !important;background: #ffffff;" value="">');
    tempTRTDs.eq(9).html('');
    createLateEvent();
}
